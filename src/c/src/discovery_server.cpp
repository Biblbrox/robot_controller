...
int run_discovery_server_impl(
  ...
  listener->set_participant_discovery_callback(
      participant_callback, user_data_on_discovery_participant);
  listener->set_reader_discovery_callback(reader_callback,
                                          user_data_on_discovery_reader);
  listener->set_writer_discovery_callback(writer_callback,
...
static void kill_server_callback(ParticipantData participant_data,
                                 void *user_data) {
...
int is_discovery_running_impl(uint32_t domain_id) {
  ...
  if (run_discovery_server_impl(domain_id, nullptr, nullptr, nullptr) == -1)
    return 1;
